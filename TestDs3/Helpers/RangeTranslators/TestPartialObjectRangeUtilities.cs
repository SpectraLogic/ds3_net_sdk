...
namespace TestDs3.Helpers.RangeTranslators
{
    ...
    {
        private static readonly IEnumerable<Range> Blobs = new[]
        {
        ...
        private static readonly IEnumerable<Range> Parts1 = new[]
        {
            Range.ByLength(5L, 10L),
            Range.ByLength(15L, 20L),
        };

        private static readonly IEnumerable<Range> Parts2 = new[]
        {
            Range.ByLength(7L, 10L),
            Range.ByLength(18L, 20L),
        };

        private static readonly IEnumerable<ContextRange<Range>> ExpectedResults1 = new[]
        {
        ...
        private static readonly IEnumerable<ContextRange<Range>> ExpectedResults2 = new[]
        {
        ...
        {
            Assert.AreEqual(
                ExpectedResults1.Sorted().ToArray(),
                PartialObjectRangeUtilities.RangesForObjectRequests(Blobs, Parts1).Sorted().ToArray()
            );
            Assert.AreEqual(
                ExpectedResults2.Sorted().ToArray(),
                PartialObjectRangeUtilities.RangesForObjectRequests(Blobs, Parts2).Sorted().ToArray()
            );
        }
        ...
        public void RangesForRequestsReturnsExpectedLookup()
        {
            var result = PartialObjectRangeUtilities.RangesForRequests(
                Blobs.Select(p => new Ds3.Helpers.Blob(p, "bar"))
                    .Concat(Blobs.Select(p => new Ds3.Helpers.Blob(p, "foo"))),
                Parts1.Select(p => new Ds3PartialObject(p, "bar"))
                    .Concat(Parts2.Select(p => new Ds3PartialObject(p, "foo")))
            );
            CollectionAssert.AreEqual(
                (
                    from cr in ExpectedResults1
                    let blob = new Ds3.Helpers.Blob(cr.Context, "bar")
                    orderby blob, cr.Range
                    select new {Blob = blob, cr.Range}
                ).Concat(
                    from cr in ExpectedResults2

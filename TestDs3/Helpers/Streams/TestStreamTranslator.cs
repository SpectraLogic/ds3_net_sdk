    ...
    public class TestStreamTranslator
    {
        ...
        public void StreamCanReadFromCorrectPlaces()
        {
            ...
            using (var resourceStore = new ResourceStore<string, Stream>(key => streams[key]))
            {
                ...
                {
                    using (
                        var stream = new StreamTranslator<string, string>(translator.Object, resourceStore, context.Key,
                            context.Sum(it => it)))
                    {
                        foreach (var payloadSize in context)
                        {
                            ...
                            results.Enqueue(Tuple.Create(context.Key,
                                Encoding.GetString(buffer, byteIndex, payloadSize)));
                        }
                    }
                }
            }
        ...
        public void StreamCanWriteToCorrectPlaces()
        {
            ...
            using (
                var resourceStore =
                    new ResourceStore<string, Stream>(key => streams.GetOrAdd(key, k => new MockStream())))
            {
                var buffer = new byte[100];
                var rand = new Random();
                foreach (var context in payloads)
                {
                    using (
                        var stream = new StreamTranslator<string, string>(translator.Object, resourceStore, context.Key,
                            context.Sum(it => it.Length)))
                    {
                        foreach (var payload in context)
                        {
                            var byteIndex = rand.Next(40);
                            Encoding.GetBytes(payload, 0, payload.Length, buffer, byteIndex);
            ...
            CollectionAssert.AreEquivalent(
                ...
                streams.Select(kvp => new {kvp.Key, Result = Encoding.GetString(kvp.Value.Result)})
            );
        }
    }
}